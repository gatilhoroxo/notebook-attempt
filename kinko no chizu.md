
# 🗺️ Kinko no Chizu

_Meu mapa para navegar pelo cofre_

---

## 🧭 Áreas Principais

### [[Areas/Programacao/index|Programação]]

Linguagens, estruturas de dados, algoritmos

- Conceitos ativos: X
- Último estudo: [[conceito-recente]]

### [[Areas/Matematica/index|Matemática]]

Fundamentos matemáticos da computação

- Conceitos ativos: X
- Último estudo: [[conceito-recente]]

### [[Areas/Sistemas/index|Sistemas]]

Sistemas operacionais, redes, databases

- Conceitos ativos: X
- Último estudo: [[conceito-recente]]

### [[Areas/Teoria-da-Computacao/index|Teoria da Computação]]

Linguagens formais, compiladores, complexidade

- Conceitos ativos: X
- Último estudo: [[conceito-recente]]

### [[Areas/Hardware/index|Hardware]]

Eletrônica, arquitetura, circuitos

- Conceitos ativos: X
- Último estudo: [[conceito-recente]]

---

## 🚀 Projetos Ativos

- [[projeto-1]] - Status: Em andamento
- [[projeto-2]] - Status: Pausado

---

## 📚 Semestre Atual

**Período:** 202X.X

**Matérias:**

- [ ] Matéria 1 - [[Areas/Programacao/conceito-x]]
- [ ] Matéria 2 - [[Areas/Matematica/conceito-y]]
- [ ] Matéria 3 - [[Areas/Sistemas/conceito-z]]

---

## 💡 Ideias Recentes

Ver: [[Ideias/index]]

Últimas 5:

1. [[ideia-1]]
2. [[ideia-2]]
3. [[ideia-3]]

---

## 📥 Inbox

Notas para organizar: X

---

## 🌱 Conceitos em Crescimento

Conceitos que estou estudando agora:

- [[conceito-1]] 🌱
- [[conceito-2]] 🌿
- [[conceito-3]] 🌳

---

## 🔥 Revisões Pendentes

#revisao

```dataview
LIST
FROM #revisao
WHERE date(file.mday) < date(today) - dur(7 days)
SORT file.mday DESC
```

---

## 📊 Estatísticas

- Total de conceitos: 
- Total de projetos: 
- Última atualização do mapa: {{date}}

---

_"O mar é vasto, mas navegar é preciso!" - Adaptação livre_